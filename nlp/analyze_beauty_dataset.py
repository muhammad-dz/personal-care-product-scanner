"""
Analyze beauty product reviews from downloaded dataset
"""
import pandas as pd
import glob
import os
...
try:
    ...
    print("⚠️ Could not import sentiment_analyzer. Creating a local version...")
    ...
    class CosmeticSentimentAnalyzer:
        def __init__(self):
            try:
                ...
                self.sia = None
                print("⚠️ NLTK not available, using simple rule-based sentiment")
...
class BeautyDatasetAnalyzer:
    ...
    def find_latest_dataset(self):
        """Find the most recent beauty dataset file"""
        patterns = [
            "beauty_reviews_*.csv",
    ...
    def analyze_dataset(self, dataset_path=None):
        """Analyze the beauty dataset"""
        if dataset_path is None:
            dataset_path = self.find_latest_dataset()
        
        if not dataset_path:
            print("❌ No dataset files found!")
            print("   Run downloader first: python scraper/download_beauty_dataset.py")
            return None
        ...
        if 'text' not in df.columns:
            if 'review' in df.columns:
                ...
                print("⚠️ No text column found, cannot analyze")
                return None
        ...
        # Ensure sentiment_percentages exists
        ...
        print(f"\n📄 Analyzed data saved to: {analyzed_file}")
    ...
    def _print_report(self, report):
        ...
        print("\n" + "="*60)
        print("📊 BEAUTY PRODUCT SENTIMENT ANALYSIS REPORT")
        print("="*60)
        ...
        print(f"  Positive: {pos} ({pos_pct}%)")
        print(f"  Neutral:  {neu} ({neu_pct}%)")
        print(f"  Negative: {neg} ({neg_pct}%)")
...
def main():
    ...
    if latest:
        ...
        choice = input("Analyze this dataset? (y/n): ").strip().lower()
